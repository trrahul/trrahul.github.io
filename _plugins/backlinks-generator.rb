...
module Jekyll
  ...
  class BacklinksGenerator < Generator
    ...
    def generate(site)
      ...
      all_documents.each do |doc|
        ...
        all_links.each do |link_info|
          ...
          target_doc = url_to_doc[normalized_link] ||
                      url_to_doc[normalized_link.chomp('/')] ||
                      url_to_doc[normalized_link.downcase] ||
                      url_to_doc[normalized_link.chomp('/').downcase]
    ...
    def skip_document?(doc)
      return true if doc.data['skip_backlinks']

      ext = doc.respond_to?(:extname) ? doc.extname.to_s.downcase : ''
      # JSON payloads surface link-like strings (e.g. series summaries) that are not actual content pages.
      return true if ext == '.json'
